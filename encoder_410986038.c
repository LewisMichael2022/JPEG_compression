...
FILE*encodeRLE_txt(codebook*dict,short*RLE,FILE*fptr);
//資料結構建立相關 
...
double compress_ratio(short***RLE_list,double**data,int M,int N,int H,int W);
//Huffman相關 
...
void save_codebook_txt(FILE**word_fptr,short*vals,char**bitstream,int stream_ptr,Huffman_node*node,char*buff,int buff_ptr);
int main(int argc, char **argv){
	if(argv[1][0]=='0'){
		...
		//input quantization table into txt file
		...
		*/
		...
		//Compute and access quantization results
		...
		//return 0;
		//Compute and access quantization error 
			...
			for(n=0;n<N;n++){
				...
				for(u=0;u<8;u++){
					...
					for(v=0;v<8;v++){
						...
						ytmp=y_F[m][n][u][v]-q_y_F[m][n][u][v]*y_Q[u][v];
						e_y[m][n][u][v]=ytmp;
						fwrite(&ytmp,sizeof(float),1,e_y_fptr);
						cbtmp=cb_F[m][n][u][v]-q_cb_F[m][n][u][v]*cbcr_Q[u][v];
						e_cb[m][n][u][v]=cbtmp;
						fwrite(&cbtmp,sizeof(float),1,e_cb_fptr);
						crtmp=cr_F[m][n][u][v]-q_cr_F[m][n][u][v]*cbcr_Q[u][v];
						e_cr[m][n][u][v]=crtmp;
		...
		if(argv[3][0]=='b'){//encoder 2 Kimberly.bmp binary rle_code.bin 
			...
			//read image
			Img img=ImRead(argv[2]);
			int M=img.header.height/H;
			int N=img.header.width/W;
			...
			free_3D_short(q_y_F_zigzag,M,N,8*8);
			free_3D_short(q_cb_F_zigzag,M,N,8*8);
			free_3D_short(q_cr_F_zigzag,M,N,8*8);
			...
			//compress ratio	
			printf("compress ratio of Y channel:");
			double Y_size=compress_ratio(y_RLE_lists,y_data,M,N,img.header.height,img.header.width);
			printf("compress ratio of cb channel:");
			double cb_size=compress_ratio(cb_RLE_lists,cb_data,M,N,img.header.height,img.header.width);
			printf("compress ratio of cr channel:");
			double cr_size=compress_ratio(cr_RLE_lists,cr_data,M,N,img.header.height,img.header.width);
			
			//total compress ratio
			printf("total compress ratio: %lf",(img.header.bitmap_headersize+Y_size+cb_size+cr_size)/img.header.filesize);
		...
		if(argv[3][0]=='a'){//encoder 3 Kimberly.bmp ascii codebook.txt huffman_code.txt 
			...
			*/
			...
			codebook dict=huffmandict(y_RLE_lists,cb_RLE_lists,cr_RLE_lists,M,N,argv[4]);
			//printf("dict finish\n");
			...
			for(m=0;m<M;m++){
				...
				for(n=0;n<N;n++){
					//printf("%d %d\n",m,n);
					fptr=encodeRLE_txt(&dict,y_RLE_lists[m][n],fptr);
					fptr=encodeRLE_txt(&dict,y_RLE_lists[m][n],fptr);
					fptr=encodeRLE_txt(&dict,y_RLE_lists[m][n],fptr);
				}
			}
...
double compress_ratio(short***RLE_list,double**data,int M,int N,int H,int W){
	int o_size=H*W*sizeof(unsigned char);
	int RLE_size=0;
	int m,n;
	for(m=0;m<M;m++){
		for(n=0;n<N;n++){
			...
			while(1){
				...
				if(ptr>0&&RLE_list[m][n][ptr]==0&&RLE_list[m][n][ptr-1]==0){
					break;
				}
				ptr++;
			}
	...
	//printf("%lf\n",RLE_size);
	double res=(double)RLE_size/o_size;
	printf("%lf\n",res);
	return (double)RLE_size;
}
...
void save_codebook_txt(FILE**word_fptr,short*vals,char**bitstream,int stream_ptr,Huffman_node*node,char*buff,int buff_ptr){
	if(node->left==NULL&&node->right==NULL){
		if(buff_ptr==-1){
			printf("error in buff_ptr\n");
		}
		//input codeword
		fprintf(*word_fptr,"%d ",node->val);
		int i=0;
		while(i<=buff_ptr){
			int res=0;
			int count=1;
		...
		vals[stream_ptr]=node->val;
		bitstream[stream_ptr]=(char*)malloc((buff_ptr+1+1)*sizeof(char));
		i=0;
		while(i<=buff_ptr){
			bitstream[stream_ptr][i]=buff[i];
			i++;
		}
		bitstream[stream_ptr][i]='2';//represent end of bit stream
		stream_ptr++;
		...
		buff[buff_ptr+1]='1';
		save_codebook_txt(word_fptr,vals,bitstream,stream_ptr,node->right,buff,buff_ptr+1);
		buff[buff_ptr+1]='0';
		save_codebook_txt(word_fptr,vals,bitstream,stream_ptr,node->left,buff,buff_ptr+1);
	}
}
...
codebook huffmandict(short***y,short***cb,short***cr,int M,int N,char*word_filename){
	...
	short*record=(short*)calloc(amount,sizeof(short));
	for(i=0;i<M;i++){
		for(j=0;j<N;j++){
			for(c=0;c<64;c++){
	...
	short*vals=(short*)malloc(node_num*sizeof(short));
	char**bitstream=(char**)malloc(node_num*sizeof(char*));
	save_codebook_txt(&word_fptr,vals,bitstream,0,head,buff,-1);
	codebook dict;
	dict.vals=vals;
	dict.bitstream=bitstream;
...
FILE*save_8x8_RLE_bin(FILE*fptr,short*list){
	...
	int ptr=1;
	while(1){
		//printf("%d %d ",skip->val,ptr->val);
		/*fwrite(&skip->val,sizeof(short),1,fptr);
		fwrite(&ptr->val,sizeof(short),1,fptr);*/
		//skip=ptr->next;
		/*if(skip==NULL)break;
		ptr=skip->next;*/
...
void bgr2ycbcr(Img*img,double**y_data,double**cb_data,double**cr_data){
	...
	for(i=0;i<(img->header).height;i++){
		for(j=0;j<(img->header).width;j++){
			int B=(img->pixels[i][j]).data[0];
			int G=(img->pixels[i][j]).data[1];
			int R=(img->pixels[i][j]).data[2];
